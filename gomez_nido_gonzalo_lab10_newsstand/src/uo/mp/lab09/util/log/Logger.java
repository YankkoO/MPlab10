package uo.mp.lab09.util.log;

import java.io.FileNotFoundException;
import java.io.FileOutputStream;
import java.io.PrintStream;
import java.io.UncheckedIOException;
import java.util.Date;

/**
 * A very basic implementation of a logger utility.
 * For this the date are sent to the System.err standard output.
 * The format of every lines is: <timestamp> <message>
 */
public class Logger {

	private final static String LOG_FILE_NAME = "newsstand.log";
	private static PrintStream out = createPrintStream();

	/**
	 * Sends the string received as message to the log, prefixing it with 
	 * a timestamp 
	 * @param message
	 */
	public static void log(String message) {
		out.print( new Date()  + " ");
		out.println( message );
	}

	private static PrintStream createPrintStream() {
		try {
			return new PrintStream(new FileOutputStream(LOG_FILE_NAME, true /*TRUE --> APPEND MODE*/));
		} catch (FileNotFoundException e) {
			throw new UncheckedIOException(e);
		}
	}

	/**
	 * Sends the full stack trace of the exception received to the log
	 * prefixing it with a timestamp 
	 * @param t, the exception to be logged
	 */
	public static void log(Throwable t) {
		out.print( new Date()  + " ");
		t.printStackTrace( out );
	}
}
